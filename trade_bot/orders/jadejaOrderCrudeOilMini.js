...
const orderFilePath = "orderFile\\crudeOilMiniOrder.json"
const fs = require("fs")

const orderReader = async () => {
   // console.log("order reader run")
   return await fs.promises.readFile(orderFilePath, "utf8", (err, jsonString) => {
      if (err) {
...
const placeOrderHandler = async (transaction_type) => {
   try {
      const placeOrder = await kiteConnection.placeOrder(tradingsymbol, transaction_type, "MCX", 1)
...
module.exports.jadejaOrderHandler = async (ary, inst_token, trading_symbol, exchange, lot_size) => {
   try {
      // console.log(ary)
      ...
      if (file_order_type === "BUY" && lastTwoAry[1][4] === "red" && lastGreenCandle[2] > lastTwoAry[1][3]) {
         orderplaced = true
      ...
      } else if (file_order_type === "SELL" && lastTwoAry[1][4] === "green" && lastRedCandle[1] < lastTwoAry[1][3]) {
      ...
      const filteredPositions = positions.filter((e) => e.tradingsymbol === tradingsymbol)
