...
module.exports.oneHourlyCrudeOilMiniDesicionMaker = async (
  instrument_token,
  trading_symbol,
  exchange,
  lot_size
  ...
  try {
    ...
    fs.writeFile(
      `${logFilePath}${new Date().getDate()}-${new Date().getHours()}-2.json`,
      JSON.stringify([...arry, lastHourCandle]),
      function (err) {
        if (err) throw err;
        console.log("2 Saved!");
      }
    ...
    console.log(JSON.stringify(heikincandleData));
    jadejaOrderCrudeOIlMini.jadejaOrderHandler(
      heikincandleData,
      instrument_token,
      trading_symbol,
      exchange,
