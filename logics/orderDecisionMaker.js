...
module.exports.crudeOilMiniOrderHandler = async (ary, stock) => {
   try {
      ...
      if (orderType === "BUY" && lastTwoAry[1][4] === "red" && lastGreenCandle[2] > lastTwoAry[1][3]) {
         ...
         const result = await Stock.updateOne({ _id: stock._id }, { $set: { status: orderType } })
      ...
      } else if (orderType === "SELL" && lastTwoAry[1][4] === "green" && lastRedCandle[1] < lastTwoAry[1][3]) {
...
module.exports.lastFiveMinuteBankNiftyFiftyOrderHandler = async (ary, stock) => {
   try {
      // console.log(ary)
      const arrivedAry = ary.length > 0 && [...ary.slice(-2)]
      const lastTwoAry = [...arrivedAry]
